...
function GridItemIcon({ type }: { type: GridIcon }) {
    const commonClass = "h-3.5 w-3.5";

    const iconMap: Record<GridIcon, React.ReactNode> = {
        ...
        target: <Globe2 className={commonClass} />,
        file: <FileText className={commonClass} />,
        map: <MapPin className={commonClass} />,
        google: <span className="text-[11px] font-bold text-blue-500">G</span>,
        factor: <span className="text-[11px] font-bold text-red-500">F</span>,
...
export default function GridTable() {
    ...
    const [rows, setRows] = useState<GridRow[]>(gridData);
                    ...
                    <thead>
                        <tr className="border-b border-slate-200 text-left text-xs font-medium text-slate-700">
                            <th className="w-[56%] px-5 py-3">
                                ...
                                >
                                    Name
                                    <span className={cn("transition-transform", !sortAsc && "rotate-180")}>
                                        ↑
                                    </span>
                                </button>
                            </th>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex -space-x-2">
                                            {row.icons.map((icon, iconIndex) => (
                                                <GridItemIcon key={`${row.id}-${iconIndex}`} type={icon} />
                                            ))}
                                        </div>
